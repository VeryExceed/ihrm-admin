          ...
          <el-tab-pane label="个人详情">
            <el-row type="flex" justify="end">
              <el-tooltip content="打印个人基本信息">
                <router-link
                  :to="`/employees/print/${userId}?type=personal`"
                >
...
export default {
  ...
  data() {
    return {
      userInfo: 'user-info',
      jobInfo: 'job-info',
      userId: this.$route.params.id, // 可以直接 通过this.userId 进行获取数据
      formData: {
        username: '',
        password2: ''
      },
  ...
  methods: {
    ...
    async saveUser() {
      try {
        ...
        await saveUserDetailById({ ...this.formData, password: this.formData.password2 }) // 将新密码的值替换原密码的值
