  ...
  methods: {
    ...
    async getPersonalDetail() {
      ...
      if (this.formData.staffPhoto.trim()) {
        // 这里我们赋值， 同事需要给赋值的地址一个标记 upload: true
    ...
    async savePersonal() {
      ...
      // 空字符串存不上 必须存个空格
      const url = fileList.length ? fileList.map(item => item.url).join(',') : ' '
      await updatePersonal({ ...this.formData, staffPhoto: url })
      this.$message.success('保存用户详情成功')
    },
    async saveUser() {
    // 去读取员工上传的头像
